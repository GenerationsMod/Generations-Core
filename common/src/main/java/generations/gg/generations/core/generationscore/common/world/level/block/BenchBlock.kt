...
class BenchBlock(properties: Properties) : GenericRotatableModelBlock<GenericModelProvidingBlockEntity>(
    properties = properties,
    blockEntityFunction = GenerationsBlockEntities.GENERIC_MODEL_PROVIDING,
    model = GenerationsBlockEntityModels.BENCH,
    width = 1
), SittableBlock {
    ...
    override fun codec(): MapCodec<BenchBlock> = CODEC
    ...
    public override fun useWithoutItem(
        ...
        player: Player,
        hit: BlockHitResult
    ): InteractionResult = super.sit(state, level, pos, player, hit).toInteractionResult();
    ...
    companion object {
        ...
        val CODEC: MapCodec<BenchBlock> = simpleCodec(::BenchBlock)
    }
}
