    ...
    fun init() {
        ...
        InteractionEvent.RIGHT_CLICK_BLOCK.register { player, hand, pos, _ ->
            ...
            if (player is ServerPlayer) {
                if (hand == InteractionHand.MAIN_HAND && player.level().getBlockState(pos).`is`(GenerationsShrines.TAPU_SHRINE.get()) && player.getItemInHand(hand).`is`(GenerationsItems.SPARKLING_STONE.get())) {
                    val dialogue: Dialogue = Dialogues.dialogues[GenerationsCore.id("tapu_spawn")] ?: return@register EventResult.interruptFalse()

                    player.openDialogue(dialogue)

                    return@register EventResult.interruptTrue()
                } else if (hand == InteractionHand.MAIN_HAND && player.level().getBlockState(pos).`is`(GenerationsShrines.ABUNDANT_SHRINE.get()) && player.getItemInHand(hand).`is`(GenerationsItems.REVEAL_GLASS.get())) {
                    val dialogue: Dialogue = Dialogues.dialogues[GenerationsCore.id("therian_spawn")] ?: return@register EventResult.interruptFalse()
