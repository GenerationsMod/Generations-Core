...
object GenerationsUtilityBlocks: BlockPlatformRegistry() {
    ...
    val BALL_LOOTS: MutableList<BallLootBlock> = ArrayList()
    ...
    val PC = registerDyed("pc") { color: DyeColor, dyeColorRegistrySupplierMap: Map<DyeColor, Block> -> DyedPcBlock(Properties.of().noOcclusion().requiresCorrectToolForDrops().strength(2f).lightLevel(PcBlock.Companion::lumiance), color, dyeColorRegistrySupplierMap) }
    ...
    ): T = GenerationsItems.ITEMS.create(name.generationsResource(), itemSupplier.invoke(Item.Properties()))

    private fun <T : Block> registerBlockItem(name: String, blockSupplier: T): T {
        val block = registerBlock(name, blockSupplier)
        register(name) { properties -> generateBlockItem(block, properties) }
        return block
    }

private fun registerLoot(name: String, ball: PokeBall): BallLootBlock {
    ...
    private fun <T : Block> registerBlock(name: String, blockSupplier: T): T {
        return registerBlock(this, name, blockSupplier)
    }

    private fun registerDyed(name: String, blockSupplier: BiFunction<DyeColor, MutableMap<DyeColor, Block>, Block>): DyedGroup {
        ...
        DyeColor.entries.forEach { dyeColor: DyeColor ->
            ...
            register(properName) { BlockItem(block, it) }
