...
import generations.gg.generations.core.generationscore.world.dialogue.nodes.ConsumerNode
...
//        LangTooltip.appendHoverText(stack, level, tooltipComponents, isAdvanced)
    ...
    override fun use(level: Level, player: Player, usedHand: InteractionHand): InteractionResultHolder<ItemStack> {
        ...
        if(player is ServerPlayer && player.isShiftKeyDown) {
            val graph = DialogueGraph(createDialogue(stack, player))
            DialoguePlayer(graph, null, player, false)
    ...
    private fun createDialogue(stack: ItemStack, player: ServerPlayer): AbstractNode {
        ...
        if(stack.damageValue == 0) return SayNode(listOf("You lack any zygarde cells."), null);
        ...
        var choice = ChooseNode("gui.zygarde_cube".asTranslated().string, nodeMap)
...
//                    it.sendSystemMessage(("$name.accept").asTranslated(), false)
        ...
        map["$name.name".asTranslated().string] = SayNode(listOf("$name.accept".asTranslated().string), ConsumerNode(consumer))
    }
