...
class MelodyFluteContainer : SingleSlotContainer {
    ...
    constructor(id: Int, playerInventory: Inventory, player: Player) : super(
        GenerationsContainers.MELODY_FLUTE.value(), id, SimpleItemStorage(player.inventory.getSelected(), IMBUED, 1)
    ) {
        slot = playerInventory.selected
        applyPlayerInventory(playerInventory)
    }

    constructor(id: Int, playerInventory: Inventory) : super(GenerationsContainers.MELODY_FLUTE.value(), id) {
        this.slot = -1
        applyPlayerInventory(playerInventory)
    }
