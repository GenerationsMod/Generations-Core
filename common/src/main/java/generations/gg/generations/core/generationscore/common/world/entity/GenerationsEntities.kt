...
object GenerationsEntities: PlatformRegistry<EntityType<*>>() {
    ...
    val BOAT_ENTITY: EntityType<GenerationsBoatEntity> = createEntityType("boat", MobCategory.MISC, EntityType.BOAT.width, EntityType.BOAT.height, ::GenerationsBoatEntity)
    ...
    var ZYGARDE_CELL: EntityType<ZygardeCellEntity> = register("zygarde_cell", EntityType.Builder.of(::ZygardeCellEntity, MobCategory.MISC).build("zygarde_cell"))

    fun <T: Entity> register(name: String, type: EntityType<T>): EntityType<T> = create(name.generationsResource(), type)
    ...
    private fun <T : Entity> createEntityType(
        ...
        return create(name.generationsResource(), EntityType.Builder.of(factory, category).sized(width, height).build(name))
    }

    override fun init(consumer: (ResourceLocation, EntityType<*>) -> Unit) {
