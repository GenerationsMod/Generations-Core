...
public class Caught extends PlayerDataExtension {
    ...
    public PlayerDataExtension deserialize(@NotNull JsonObject jsonObject) {
        ...
        jsonObject.entrySet().stream().filter(a -> !a.getKey().equals(Companion.getNAME_KEY())).forEach(entry -> multiset.setCount(SpeciesKey.fromString(entry.getKey()), entry.getValue().getAsInt()));
