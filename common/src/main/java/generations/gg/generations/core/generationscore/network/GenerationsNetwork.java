...
public class GenerationsNetwork implements GenerationsImplementation.NetworkManager {
    ...
    public void registerClientBound() {
        this.createClientBound(S2COpenMailEditScreenPacket.ID, S2COpenMailEditScreenPacket.class, S2COpenMailEditScreenPacket::decode, new S2COpenMailEditScreenPacket.Handler());
    ...
    private <T extends GenerationsNetworkPacket<T>> void createClientBound(ResourceLocation identifier, Class<T> kClass, Function<FriendlyByteBuf, T> decoder, generations.gg.generations.core.generationscore.network.ClientNetworkPacketHandler<T> handler) {
        createClientBound(identifier, kClass, T::encode, decoder, handler);
    }
